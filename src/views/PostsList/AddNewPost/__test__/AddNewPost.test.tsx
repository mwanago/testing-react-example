  ...
  describe('useAddNewPost', () => {
    test('should not crash when called', () => {
      expect(() => {
        ...
        const hookValue = renderHook(() =>
          useAddNewPost(updatePostsWithNewPostMock),
        );
      }).not.toThrow();
    });
    ...
    describe('onCreateNewPostSubmit', () => {
      test('should call postsApi.createNewPost with form values', async () => {
        const updatePostsWithNewPostMock = vi.fn();
        const formValues: CreateNewPostDTO = {
          body: '12',
          title: 'abc',
          userId: 12,
        };
        const useAddNewPostRender = renderHook(
          () => useAddNewPost(updatePostsWithNewPostMock),
          { wrapper: TestWrapper },
        );
        await useAddNewPostRender.result.current.onCreateNewPostSubmit(
          formValues,
        );
      ...
      test('should set isLoading value before api call to true and after api call to false', async () => {
        ...
        const submitPromise =
          useAddNewPostRender.result.current.onCreateNewPostSubmit(formValues);
